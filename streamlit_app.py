...
def get_all_stock_codes():
    """优先从本地CSV文件读取股票列表（快速），如果不存在则联网获取"""
    try:
...
with st.sidebar:
    st.header("⚙️ 参数设置")
    days = st.slider("历史数据天数", min_value=30, max_value=200, value=60, step=10)
    min_score = st.slider("最低评分阈值", min_value=0, max_value=100, value=60, step=5)
    max_workers = st.slider("并行线程数", min_value=2, max_value=16, value=6, step=2)
